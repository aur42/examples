...
class SphereApi {
    ...
    static createUsageRecord = async(subscriptionItemId: string, quantity: number) => {
        const usageRecord = (await axios.post(
            `${API_URL}/usageRecord`,
            {
    ...
    static createUsageRecordsForSubscription = async(subscription: any) => {
        ...
        if (subscriptionItems.length < 1) throw Error("No subscription items found.")
        return Promise.all(subscriptionItems.map((subscriptionItem: any) => 
...
class Handler {
    static subscriptionCreate = async (body: any): Promise<any> => {
        ...
        console.log(`A customer has created a ${subscription.id}. Waiting on the ${subscription.network} network for confirmation.`)
        return []
    }

    static subscriptionApprove = async (body: any) => {
        const subscription = body?.data?.subscription;
        console.log(`Approval for ${subscription.id} has been confirmed from the ${subscription.network} network.`)
    ...
    static subscriptionPastDue = async(body: any): Promise<any[]> => {
        const subscription = body?.data?.subscription;
        console.log(`Ugh oh, ${subscription.id} was unable to meet their payment this billing cycle. Putting them in pastDue.`)
        return []
    }
}
...
app.post(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    console.log(`In response to ${eventId} usage records created: `, usageRecords)
