...
const config = {
  headers: { Authorization: "Bearer secret_043de2ea0dd7406b8a11b67f283a27d4" },
};
...
const createBankAccount = async (args: CreateBankAccount) => {
  const response = axios
    .post(`${API_URL}/bankAccount`, args, config)
...
const listBankAccount = async () => {
  const response = await axios.get(`${API_URL}/bankAccount`, config);
  if (response.status !== 200) {
    throw new Error(response.data);
  }
...
const createPayout = async (args: CreatePayout) => {
  const response = await axios.post(`${API_URL}/payout`, args, config);
  if (response.status !== 200) {
    throw new Error(response.data);
  }
