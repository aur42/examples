...
const isValidPayload = (req: express.Request) => {
  const signingSecret: string | undefined = process.env.SPHERE_WEBHOOK_SIGNING_SECRET;
  if (!signingSecret) throw new Error("SPHERE_WEBHOOK_SIGNING_SECRET is not set. Please set it in your env.");
  const body = req.body;
