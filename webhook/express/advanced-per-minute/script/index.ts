...
const pauseActiveSubscriptions = async() => {
    ...
    return Promise.all(subscriptions
        ...
        .map((subscription: any) => axios.post(
...
const createProduct = async() => (await axios.post(
    `${API_URL}/product`,
    {
        name: "Advanced Per Minute",
        description: "A per metered subscription that reacts to subscription billing events to update usage."
    },
    config
))?.data?.data?.product

const createPrice = async(productId: string) => (await axios.post(
    `${API_URL}/price`,
    {
        product: productId,
        unitAmountDecimal: 1,
...
const createPaymentLink = async(priceId: string) => (await axios.post(
    `${API_URL}/paymentLink`,
    {
        lineItems: [
            {
...
const createWebhook = async() => {
    ...
    const existingWebhook = webhooks.find((webhook: any) => webhook.name == "advanced-per-minute")
    if (existingWebhook) {
        // Delete the webhook if it already exists
        await axios.delete(
            `${API_URL}/webhook/${existingWebhook.id}`,
            config
        )
    }
